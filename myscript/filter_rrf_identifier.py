...
if not os.path.exists(args.input):
    print(f"Input file '{args.input}' does not exist. Stopping the script.")
else:
    table = pd.read_csv(args.input, delimiter='\t')
    look = table.value_counts()
    look = pd.DataFrame(look).reset_index()
    look.columns = ['Category', 'rRNA_info', 'Gene_Start', 'Gene_End', 'Sequence', 'Length', 'Genomic_Start',
                    'Genomic_End', 'Difference', 'count', 'RPM', 'Seq_count']
    ...
    # to find rRFs seq abundance in each rRNA 
